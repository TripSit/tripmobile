...
cat ../TripMobile/app/src/main/res/values/strings.xml | sed 's/^.*<!--.*getString.*-->.*$//g' | sed 's/.*">//g' | sed 's/<\/string>//g' | sed 's/\/string>//g' | sed 's/^ *//g' | sed 's/\\n//g' | sed 's/%1\$s/___/g' | sed 's/\\'/'/g' | sed 's/<\/item>$//g' | sed 's/<\/plurals>$//g' | sed 's/<!\[CDATA\[//g' | sed 's/\]\]>//g' | sed -n -e '/#######/,$p' | grep -B 1000000 '######' | sed 's/^.*########.*$//g' | sed 's/^.*USER FACING STRINGS.*$//g' | sed 's/^.*The following Strings are seen by the user and need translation and proper grammar.*$//g' | sed 's/^\"//g' | sed 's/\"$//g' | cat -s >> $FILE
